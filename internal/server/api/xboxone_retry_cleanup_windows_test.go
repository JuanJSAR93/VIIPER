...
func TestStopRetry0980ABIHasExactLocationAndNeverDetachesOrStopsAll(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, uintptr(1124), unsafe.Sizeof(request))
	require.Equal(t, uintptr(1120), unsafe.Offsetof(request.Count))
	...
	calls.control = func(_ windows.Handle, code uint32, in *byte, inSize uint32, out *byte, outSize uint32, returned *uint32, _ *windows.Overlapped) error {
		...
		require.Equal(t, uint32(1124), inSize)
